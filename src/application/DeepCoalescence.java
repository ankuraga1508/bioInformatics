...
public class DeepCoalescence extends GUI{
	...
	public void compareAgainstGeneTree(File file, String speciesTree) {
		...
		subStage.setTitle("Deep Coalescence");
		...
		btn.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(final ActionEvent e) {
				try {
					...
					tab.setContent(new CostView(speciesTree+"#"+geneTree+"%", compareScore, "deep coalescence"));
...
//					alert.showAndWait();
	...
	public void askParameters(final Stage stage, boolean flag, String firstTree, File compareFile) {
		...
		final GridPane collapsableGrid = new GridPane();
		...
		collapsableGrid.add(seed, 0, 10);
		final TextField seedTextField = new TextField();
		collapsableGrid.add(seedTextField, 1, 10);
				...
				try {
					...
					ArrayList<Output> showOutput = new ArrayList<Output>();
					 if (file != null) {
							...
							addInputItem(file.toString(), "DeepCoalescene");
							showOutput = runDeepCoalescene(file);
							addResultItems(showOutput, "DC");
						}
					 else {
						...
						if(compareFile != null)
							...
							addResultItems(showOutput, "DC", tempFile);
    ...
    public ArrayList<Output> runDeepCoalescene(File file) {
    	ArrayList<Output> showOutput = new ArrayList<Output>();
    	String tree;
    	...
    	try {
        	...
        	while ((s = stdInput.readLine()) != null) {
        		...
        		if(s.contains("[Species tree ")) {
        			s = s + "\n" + stdInput.readLine() + "\n";
        			String scoreLine = stdInput.readLine();
        			bestScore= Integer.parseInt(scoreLine.replaceAll("[^0-9]", ""));
        			s += scoreLine + "\n";	
